    ...
    function getVars(refName) {
        return context.getScope().variables
            .filter(v => v.defs.some(def => {
                ...
                return name && (refName === name /*|| getVars(name).includes(refName)*/);
            }))
            .map(v => v.name);
    }
        ...
        "CallExpression": function (node) {
            ...
            disallowedMethods.forEach(function(disallowedMethod) {
                ...
                if (calledObject
                    ...
                    context.report(node, disallowedMethod.log || `Calling ${disallowedMethod.object}.${disallowedMethod.method}() is disallowed`);
                }
            });
        }
    };
